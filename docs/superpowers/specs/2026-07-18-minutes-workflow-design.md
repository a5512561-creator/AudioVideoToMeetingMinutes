...
- **`semantic[].label` 以 checklist 為準，不信 LLM 回傳**：audit prompt 要求 LLM 逐項回傳 label，但 label 的正本在 `audit_checklist.md`。LLM 可能改寫用詞。**P3 UI 顯示 label 時應以自己那份 checklist 為準（用 `key` 對應）**，不要直接渲染 `audit.json` 裡 `semantic[].label` 的 LLM 文字。
